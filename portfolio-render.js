(() => {
  ...
  const render = () => {
    ...
    if (data.pillars && document.getElementById('pillars')) {
    ...
    if (data.aiProjects && document.getElementById('ai-projects-grid')) {
      fill('ai-projects-grid', data.aiProjects.map((project, index) => `<article class="ai-card reveal" data-spot><div class="ai-card-top"><span class="ai-card-index" aria-hidden="true">${pad(index + 1)}</span><span class="ai-card-badge">${escape(project.metric)}</span></div><div class="ai-card-body"><p class="eyebrow">${escape(project.category)}</p><h3>${escape(project.title)}</h3><p>${escape(project.description)}</p></div>${tags(project.tags)}</article>`).join(''));
    }

    // Experience: a ledger of roles with progressive disclosure.
    if (data.roles && document.getElementById('timeline')) {
    ...
    if (data.framework && document.getElementById('framework')) {
    ...
    if (data.cases && document.getElementById('case-grid')) {
    ...
    if (data.cases && document.getElementById('case-preview')) {
    ...
    if (data.metrics && document.getElementById('metrics')) {
    ...
    if (data.skills && document.getElementById('skills-grid')) {
    ...
    if (data.journey && document.getElementById('journey-stages')) {
    ...
    if (insightsEl) {
      insightsEl.hidden = !data.showInsights;
      if (data.insights && document.getElementById('insights-grid')) {
